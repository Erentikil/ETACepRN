...
export default function HizliIslemler() {
  ...
  useEffect(() => {
    ...
    if (!sirket) return;
    // Sepet boş VE cari seçilmemişse temizle, aksi halde kaydet
    if (sepetKalemleri.length === 0 && !secilenCari) {
    ...
    useCallback(() => {
      ...
      aktifSepetAl(calisilanSirket).then((sepet) => {
        if (!sepet || (sepet.kalemler.length === 0 && !sepet.cariKodu)) {
          ...
          if (sepetKalemlerRef.current.length === 0 && !evrakTipiKoru.current) {
            setSecilenCari(null);
  ...
  return (
    <View style={[styles.ekran, { backgroundColor: Colors.background }]} onTouchStart={Keyboard.dismiss}>
      ...
      {filtreli.length > 0 && (
        <Text style={[styles.toplamStokText, { color: Colors.textSecondary, borderTopColor: Colors.border, backgroundColor: Colors.card }]}>Toplam: {filtreli.length} stok</Text>
      )}
...
const styles = StyleSheet.create({
  ...
  aramaTipiItemTextActive: { fontWeight: '700' },
  listeBaslik: {
    flexDirection: 'row',
    paddingHorizontal: 14,
    paddingVertical: 6,
